<x-layout>
                ...
                <div class="items-center mx-auto mb-3 space-y-4 max-w-screen-sm sm:flex sm:space-y-0">
                    <div class="relative w-full">
                        ...
                        <input
                            ...
                            placeholder="Search for articles..." type="search" id="search" name="search" value="{{ request('search') }}">
                    </div>
                    <div>
                        <button type="submit"
    ...
    {{-- Pagination Top - Hidden for infinite scroll --}}
    <div class="mb-6 pagination-controls">
        {{ $posts->links() }}
    </div>

    {{-- Modern Posts Grid with Animations --}}
            ...
            @forelse ($posts as $index => $post)
                <article
                    ...
                    <div class="flex justify-between items-center mb-4 text-gray-500 relative z-10">
                        <a href="/posts?category={{ $post->category->slug }}" class="group/cat">
                            <span
                                class="px-3 py-1 rounded-full text-xs font-bold bg-{{ $post->category->color }}-100 dark:bg-{{ $post->category->color }}-900 text-{{ $post->category->color }}-800 dark:text-{{ $post->category->color }}-200 shadow-sm hover:shadow-md transition-all duration-300 transform group-hover/cat:scale-110">
                                {{ $post->category->name }}
                            </span>
                        </a>
                    ...
                    <div class="flex justify-between items-center relative z-10">
                        <a href="/posts?author={{ $post->author->username }}" class="group/author">
    ...
    {{-- Pagination Bottom - Hidden for infinite scroll --}}
